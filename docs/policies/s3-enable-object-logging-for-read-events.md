...
This rule is covered by the [s3-enable-object-logging-for-events](https://github.com/hashicorp/policy-library-cis-aws-s3-terraform/blob/main/policies/s3-enable-object-logging-for-events.sentinel) policy with the `event_type` parameter passed as `ReadOnly`.
